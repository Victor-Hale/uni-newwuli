    ...
    public function login(Request $request)
    {
       ...
       $password=$request['password'];
        $token = auth('api')->attempt($credentials);   //获取token
         $data['token']=$token;
         $data['state1']=DB::table('experiment')->where('student_id',$stuid)->value('state');
         $data['state2']=DB::table('danbai')->where('student_id',$stuid)->value('state_danbai');
         $data['dianbiao']=DB::table('dianbiao')->where('student_id',$stuid)->value('state_dianbiao');
         $data['huoer']=DB::table('huoer')->where('student_id',$stuid)->value('state_huoer');
         $data['oumubiao']=DB::table('oumubiao')->where('student_id',$stuid)->value('state_oumu');
         $data['shiboqi']=DB::table('shiboqi')->where('student_id',$stuid)->value('state_shiboqi');
         $data['wangyongbiao']=DB::table('wangyongbiao')->where('student_id',$stuid)->value('state_wangyongbiao');
         $data['xiangshi_dianqiao']=DB::table('xiangshi_dianqiao')->where('student_id',$stuid)->value('state_xs_dianqiao');
         $data['zhiliu_dianqiao']=DB::table('zhiliu_dianqiao')->where('student_id',$stuid)->value('state_zl_dianqiao');
